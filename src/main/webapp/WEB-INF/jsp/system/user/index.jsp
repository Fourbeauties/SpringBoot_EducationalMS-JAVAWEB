...
<%--&lt;%&ndash;    <select  name=""  id="roles">&ndash;%&gt;--%>
...
<table class="layui-table" lay-data="{id:'userTable',url:'/easUser/list', page:false,toolbar:'#toolbarDemo',defaultToolbar: ['filter', 'print', 'exports'],even: true,height : 'full-125'}"
            ...
            switch (e){
                    ...
                    $.get('/easUser/form',function (str) {
                        layer.open({
                            ...
                            yes : function (index) {
                                ...
                                $.post('/easUser/add',params,function (data) {
                                    if(data.code==0) {
                                        console.log(params)
                                        layer.close(index);
                                        layer.msg('添加成功');
                ...
                case "batchDelete":
                    ...
                    layer.confirm('真的干掉这么多数码吗？',{icon:5,title:'友情提示'},function (index) {
                        ...
                        $.post('/easUser/batchDelete',params,function () {
                            layer.close(index);
                            table.reload('userTable');
                        });
                    });
            ...
            if (obj.event == 'edit'){ //edit
                $.get("/easUser/form",function (str) {
                    layer.open({
                        ...
                        btn:['确定','取消'],
                        success:function(){
                            $.get('/easUser/view',{id:data.id},function (data) {
                                //显示数据
                        ...
                        yes : function (index) {
                            ...
                            $.post('/easUser/edit',params2,function (data) {
                               if(data.code==0){
                                   // console.log(params2);
                                   layer.close(index);
                                   layer.msg('修改成功')
                ...
                layer.open({
                    ...
                    btn:['确定','取消'],
                    yes : function () {
                        $.post("/easUser/batchDelete",{"ids":data.id},function () {
                            layer.msg('删除成功');
                            table.reload('userTable');
                        });
                    }
...
<script>
    document.querySelector('.xm-select-parent').style.display = 'none';
</script>
...
<script>
    document.querySelector('.xm-select-parent').classList.add('hidden');
</script>
