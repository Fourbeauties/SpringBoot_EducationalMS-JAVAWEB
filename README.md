...
(1)课程管理：当课程已经有学生选课成功时，将不能删除
(2)学生管理：添加学生信息时，其信息也会添加到登录表中
(3)教师管理：添加教师信息时，其信息也会添加到登录表中
(4)账户密码重置：修改学生和教师的密码，不需要输入旧密码
(5）修改密码：修改自己的密码，需要输入旧密码
(6）所有学生信息： showStudent
(7）按照名字模糊查找学生信息： selectStudent
(8）添加学生信息： saveStudent
(9）修改学生信息： updateStudent
(10）删除学生信息： deleteStudent
(11）修改学生或教师的密码： updateOthersPassword
(12）修改自己的密码： updatePassword

3、教师模块功能
教师登陆后，可以查看自己教授的课程，查询选修该课程的学生，对选修该课程的学生进行打分，修改个人密码
(1）查看自己所教授的课程： showCourse
(2）查询选修该课程的学生： showStudent
(3）对选修该课程的学生进行打分： mark
(4）修改自己的密码： updatePassword

4、学生模块功能
学生登录后，可以进行选课，查看已修课程，查看已选课程，退选课程，修改个人密码
(1）所有课程: 在这里选修课程，选好后，将会自动跳转到已选课程选项： showCourse
(2）已选课程: 这里显示的是，还没修完的课程，也就是老师还没给成绩，由于还没有给成绩，所以这里可以进行退课操作： selectedCourse
(3）已修课程: 显示已经修完，老师已经给成绩的课程： 

代码部分结合branch main与master
